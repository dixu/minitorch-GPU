class Module:
    ...
    def train(self):
        ...
        for module_name in self._modules:
            self._modules[module_name].train()
    ...
    def eval(self):
        ...
        self.training = False
        for module_name in self._modules:
            self._modules[module_name].eval()
        #raise NotImplementedError('Need to implement for Task 0.4')
    ...
    def named_parameters(self):
        ...
        named_pars = []
        for _par in self._parameters.items():
            # print("****", _par[0], "*****", _par[1])
            # named_pars.append((_par[0].replace("parameter_", ""), Parameter(x=_par[1])))
            # named_pars.append((_par[0].replace("parameter_", "", 1), _par[1]))
            named_pars.append((_par[0], _par[1]))


        
        for module_name in self._modules:
            for _par in self._modules[module_name].named_parameters():
                #named_pars.append((module_name.replace("module_", "")+"."+_par[0], _par[1]))
                named_pars.append((module_name+"."+_par[0], _par[1]))
